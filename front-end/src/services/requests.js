...
const deleteById = async (id) => {
  try {
    const response = await axios.delete(`${baseUrl}/deleteOldRequest/${id}`);
...
const getAllByToken = async (webhookToken) => {
  try {
    const response = await axios.get(`${baseUrl}/${webhookToken}`);
    ...
    if (response.data) {
      ...
      while (index < response.data.length) {
        if (currentDate - new Date(response.data[index].created_at) > 60000) {
